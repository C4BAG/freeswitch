...
static void handle_ice(switch_rtp_t *rtp_session, switch_rtp_ice_t *ice, void *data, switch_size_t len)
{
		...
		case SWITCH_STUN_ATTR_CONTROLLING:
			{
				...
				uint64_t *tiebreaker = (uint64_t*)attr->value;
	...
	if (ice->type & ICE_VANILLA) {
		...
		if (stun_error == 487) {
			if ((ice->type & ICE_CONTROLLED) && controlling_controlled == SWITCH_STUN_ATTR_CONTROLLED) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING, "%s STUN Changing role to CONTROLLING\n", rtp_type(rtp_session));
				ice->type &= ~ICE_CONTROLLED;
				if (controlling_controlled_tiebreaker < ULLONG_MAX) {
					ice->tiebreaker = controlling_controlled_tiebreaker + 1;
				}
			} else if (!(ice->type & ICE_CONTROLLED) && controlling_controlled == SWITCH_STUN_ATTR_CONTROLLING) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING, "%s STUN Changing role to CONTROLLED\n", rtp_type(rtp_session));
				ice->type |= ICE_CONTROLLED;
				if (controlling_controlled_tiebreaker > 0) {
					ice->tiebreaker = controlling_controlled_tiebreaker - 1;
				}
			} else {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING, "%s STUN error 487 ignored!\n", rtp_type(rtp_session));
			}
			packet->header.type = SWITCH_STUN_BINDING_RESPONSE; // process received SWITCH_STUN_BINDING_ERROR_RESPONSE like SWITCH_STUN_BINDING_RESPONSE in the next steps
		} else if (controlling_controlled == SWITCH_STUN_ATTR_CONTROLLING && !(ice->type & ICE_CONTROLLED)) {
		...
		} else if (controlling_controlled == SWITCH_STUN_ATTR_CONTROLLED && (ice->type & ICE_CONTROLLED)) {
			if (ice->tiebreaker >= controlling_controlled_tiebreaker) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING, "%s STUN Changing role to CONTROLLING\n", rtp_type(rtp_session));
				ice->type &= ~ICE_CONTROLLED;
			}
	...
	int add_candidate = switch_rtp_ice_add_candidate(rtp_session, ice, from_host, from_port, stun_message_priority, stun_message_use_candidate);
	if (add_candidate < 0 || 1 != switch_rtp_ice_acl_check(rtp_session, ice, from_host, from_port)) {
		goto end;
	}
	if (add_candidate == 1 && stun_message_use_candidate && (ice->type & ICE_VANILLA))
		ok = 1;
	...
	if ((ice->type & ICE_VANILLA)) {
		if (!ok) ok = stun_message_use_candidate != 0;
...
static int do_dtls(switch_rtp_t *rtp_session, switch_dtls_t *dtls)
{
	...
	// Claude analysis of the following ICE checks:
	// The IP address and candidate check is overly conservative defensive code without strong security justification. It may prevent legitimate scenarios such as NAT rebinding, mobile network handover,
	// TURN relay usage, and dual-stack (IPv4/IPv6) transitions.
	// A more robust implementation would rely solely on DTLS fingerprint verification and implement rate-limiting for DoS protection instead of IP-based filtering.
	if (is_ice && !(rtp_session->ice.type & ICE_DISABLE_DTLS_PROTECTION)) {
		if (!(rtp_session->ice.type & ICE_LITE) && !rtp_session->ice.cand_responsive) {
		...
		if (!switch_cmp_addr(rtp_session->from_addr, rtp_session->ice.addr, SWITCH_TRUE)) {
			char tmp_buf1[80] = "";
			char tmp_buf2[80] = "";
