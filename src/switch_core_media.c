...
static uint32_t calc_candidate_priority_host(int protocol, int external_priority) 
{
	int local_preference_dec = 0;
	switch_assert(local_preference_dec >= 0 && local_preference_dec < 10);
	switch_assert(protocol == 1 || protocol == 2);
	switch_assert(external_priority >= -1 && external_priority <= 2); 
	if (external_priority > 0) 
		return (1 << 24) * 126 + (1 << 8) * (65525 - local_preference_dec) + (1 << 0) * (256 - protocol);
	return (1 << 24) * 126 + (1 << 8) * (65535 - local_preference_dec) + (1 << 0) * (256 - protocol);
}

static uint32_t calc_candidate_priority_external(int protocol, int external_priority)
{
	...
	if (external_priority < 0)
		return (1 << 24) * 126 + (1 << 8) * (65525 - local_preference_dec) + (1 << 0) * (256 - protocol);
	return (1 << 24) * 126 + (1 << 8) * (65535 - local_preference_dec) + (1 << 0) * (256 - protocol); // higher than "srflx" because local_preference_dec is 1 vs. 2
}

static uint32_t calc_candidate_priority_srflx(int protocol, int external_priority)
{
	int local_preference_dec = external_priority > 1 ? 2 : 0;
	switch_assert(local_preference_dec >= 0 && local_preference_dec < 10);
	switch_assert(protocol == 1 || protocol == 2);
	switch_assert(external_priority >= -1 && external_priority <= 2);
	if (external_priority > 1)
		return (1 << 24) * 126 + (1 << 8) * (65535 - local_preference_dec) + (1 << 0) * (256 - protocol); // lower than "external" host because local_preference_dec is 2 vs. 1
	return (1 << 24) * 100 + (1 << 8) * (65535 - local_preference_dec) + (1 << 0) * (256 - protocol);
}
...
static void gen_ice(switch_core_session_t *session, switch_media_type_t type, const char *ip, switch_port_t port)
{
	...
	if (!engine->ice_out.cands[0][0].component_id) {
		...
		engine->ice_out.cands[0][0].priority = calc_candidate_priority_host(engine->ice_out.cands[0][0].component_id, 0);
	}
					   ...
					   switch_sdp_type_t sdp_type)
{
	...
	if ((tmp = switch_channel_get_variable_dup(session->channel, "external_candidate_priority", SWITCH_FALSE, -1))) {
		external_candidate_priority = atoi(tmp);
		if (external_candidate_priority < -1 || external_candidate_priority > 2) external_candidate_priority = 0;
	}
	...
	if (a_engine->ice_out.cands[0][0].ready) {
		...
		if (include_external && !zstr(smh->mparams->extsipip)) {
			if (external_candidate_srflx) {
				switch_snprintf(buf + strlen(buf), buflen - strlen(buf), "a=candidate:%s 1 %s %u %s %d typ srflx raddr %s rport %d generation 0\r\n", 
					tmp3, ice_out->cands[0][0].transport, calc_candidate_priority_srflx(1, external_candidate_priority),
		...
		if (!zstr(a_engine->local_sdp_ip) && !zstr(ice_out->cands[0][0].con_addr) && 
			...
			switch_snprintf(buf + strlen(buf), buflen - strlen(buf), "a=candidate:%s 1 %s %u %s %d typ srflx raddr %s rport %d generation 0\r\n",
							tmp2, ice_out->cands[0][0].transport, calc_candidate_priority_srflx(1, external_candidate_priority),
							ice_out->cands[0][0].con_addr, ice_out->cands[0][0].con_port,
		...
		if (a_engine->rtcp_mux < 1 || switch_channel_direction(session->channel) == SWITCH_CALL_DIRECTION_OUTBOUND || switch_channel_test_flag(session->channel, CF_RECOVERING)) {
			...
			if (include_external && !zstr(smh->mparams->extsipip)) {
				if (external_candidate_srflx) {
					switch_snprintf(buf + strlen(buf), buflen - strlen(buf), "a=candidate:%s 2 %s %u %s %d typ srflx raddr %s rport %d generation 0\r\n", 
						tmp3, ice_out->cands[0][0].transport, calc_candidate_priority_srflx(2, external_candidate_priority),
						smh->mparams->extsipip, ice_out->cands[0][0].con_port,
						ice_out->cands[0][0].con_addr, ice_out->cands[0][0].con_port
						);
				}
			...
			if (!zstr(a_engine->local_sdp_ip) && !zstr(ice_out->cands[0][1].con_addr) && 
				...
				switch_snprintf(buf + strlen(buf), buflen - strlen(buf), "a=candidate:%s 2 %s %u %s %d typ srflx raddr %s rport %d generation 0\r\n",
								tmp2, ice_out->cands[0][0].transport, calc_candidate_priority_srflx(2, external_candidate_priority),
								ice_out->cands[0][0].con_addr, ice_out->cands[0][0].con_port + (a_engine->rtcp_mux > 0 ? 0 : 1),
